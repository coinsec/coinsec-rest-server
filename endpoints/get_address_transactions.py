...
async def get_transactions_for_address(
    ...
    coinsecAddress = re.sub(r"^coinsec(test)?:", "",
                          coinsecAddress)  # Custom query bypasses the TypeDecorator, must handle it manually
    async with async_session() as session:
        resp = await session.execute(
            ...
            WHERE o.script_public_key_address = :coinsecAddress
...
@app.get("/addresses/{coinsecAddress}/full-transactions-page",
         response_model=List[TxModel],
         response_model_exclude_unset=True,
         tags=["Coinsec addresses"])
@sql_db_only
...
@app.get("/addresses/{coinsecAddress}/name",
         response_model=AddressName | None,
         tags=["Coinsec addresses"])
@sql_db_only
async def get_name_for_address(
        response: Response,
        coinsecAddress: str = Path(description="Coinsec address as string e.g. "
                                             "coinsec:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e",
                                 regex=REGEX_COINSEC_ADDRESS)
):
    """
    ...
    async with async_session() as s:
        r = (await s.execute(
            select(AddressKnown)
            .filter(AddressKnown.address == coinsecAddress))).first()
