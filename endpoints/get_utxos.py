...
from constants import REGEX_COINSEC_ADDRESS, ADDRESS_EXAMPLE
...
@app.get("/addresses/{coinsecAddress}/utxos", response_model=List[UtxoResponse], tags=["Coinsec addresses"])
async def get_utxos_for_address(coinsecAddress: str = Path(
    description=f"Coinsec address as string e.g. {ADDRESS_EXAMPLE}",
    regex=REGEX_COINSEC_ADDRESS)):
    """
    Lists all open utxo for a given coinsec address
    """
    resp = await coinsecd_client.request("getUtxosByAddressesRequest",
                                       params={
                                           "addresses": [coinsecAddress]
                                       }, timeout=120)
    try:
        return (utxo for utxo in resp["getUtxosByAddressesResponse"]["entries"] if utxo["address"] == coinsecAddress)
    except KeyError:
